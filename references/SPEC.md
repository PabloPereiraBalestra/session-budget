...
- S/M/L are calibration buckets with default estimates S=5, M=12, L=25 session points. Point estimates may be any integer (8, 15, etc.); the bucket only determines which median applies during self-tuning. Defaults are overridden by measured data per the Self-tuning section. When assigning a block's point estimate during planning (including re-estimating backlog items already sitting in SESSION_STATE.md at the next resume), use the (size, model) bucket's current calibrated median from the Cost calibration section if that bucket has ≥3 qualifying actuals; only use the raw default when no calibrated median applies yet for that pair. A plan drafted before calibration data existed, or before a bucket crossed the n≥3 threshold, must not keep citing the stale default once better data is available — this is what estimating from calibration means in practice, not a new rule.
...
- After completing each block, append one line to budget_log.jsonl (project root, append-only). Capture the real local timestamp before composing the line, never a placeholder. Never edit past lines and never rewrite the file to fix one; if an appended line came out wrong, append a corrective line referencing it instead:
  {"t":"block","ts":"<ISO local>","tag":"<DESIGN|MECHANICAL>","size":"<S|M|L>","model":"<model.display_name from snapshot>","effort":"<effort.level from snapshot, omit if unavailable>","agent_model":"<display name of the model that actually executed a delegated block's subagent, omit if the block ran directly in the main thread>","est":<points>,"actual":<end_pct - start_pct>,"start_pct":<n>,"end_pct":<n>,"commit":"<hash>","clean":<true|false>}
  In manual mode (no usable snapshot): model from session context, actual/start_pct/end_pct = null. "effort" records the main thread's effort level at block close; calibration buckets remain (size, model) — the field only accumulates data for a possible future split by effort. "agent_model" records which model a delegated subagent actually ran on when it differs from the main thread's `model` (e.g. a Fable or Opus subagent dispatched from a Sonnet orchestrator); it is observability metadata only — calibration buckets stay keyed on (size, model) of the main thread, never on agent_model, until evidence justifies otherwise.
...
### Self-tuning
- On every resume, before planning: read the last 30 lines of budget_log.jsonl. Recalibrate each (size, model) estimate as the MEDIAN of its last 5 non-null actuals, excluding lines flagged "parallel":true or "spans_reset":true (they stay in the log for reporting, but never feed calibration). A bucket's median overrides the fallback level below it only once the bucket has ≥3 qualifying actuals; with fewer, fall through. Fallback order: (size, model) → size-only median (same ≥3 rule) → defaults. actual=0 lines are qualifying data like any other (with n≥3 the median absorbs them); the two flags above are the ONLY valid exclusions — never drop a line from calibration by judgment call. Write the result to the Cost calibration section of SESSION_STATE.md — this section is the single source of computed bucket medians, refreshed in the same commit as any block or session close that changes bucket membership; any other section that recaps calibration (e.g. a resume-context note) must reference those numbers rather than independently restate them. Silent, no announcement needed.
- Second-order rules, only once the log has ≥10 block entries with non-null actuals (before that: calibration phase, first-order only). On every resume, after recalibrating, state explicitly whether second-order rules are active (the ≥10 threshold) and, when active, evaluate and report all three conditions below even when none trigger — a threshold that activates silently is a threshold that gets skipped silently:
